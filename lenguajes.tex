...
\begin{ej} Consideremos los siguientes ejemplos.	
	\begin{enumerate}[E1.]
		...
		Representamos al autómata de la siguiente manera donde $b_{i}, b_{j} \in B$ entonces si $b_{i} \cdot b_{j}$ es el producto en $F$ de ambos generadores nuestro automáta lo describimos de la siguiente manera
		...
		\begin{center}
			\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=6.5cm,
				...
				\path 
				(q_1) edge    [bend right,swap]          node {$b_{i}, b_{j} \mid b_{i} \cdot b_{j} $ \ } (q_0)
		...
		Notemos que este automáta es determinístico.				
		Consideramos el lenguaje aceptado por estado final, esto es:
		\[
		L({\cal M}) = \{  w \in B^* \mid (q_{0},w,1)   \vdash^*  (q_{0}, \epsilon, y)  \}.
		\]
		
		Probamos las siguientes dos observaciones:
		\begin{enumerate}[a)]
			\item Sea $w = w_{1}w_{2} \in B^*$ luego 
			$(q_{0},w,1) \overset{*}{\vdash} (q,w_{2},\pi(w_{1}))$ si $q \in \{q_{0},q_{1}\}$.
			
			Para probar esto lo hacemos por inducción en la longitud de la derivación.
			Supongamos que $w = bw'$ con $w' \in B^*$ caso contrario no hay nada que probar.
			Para el caso base tenemos que $(q_{0},bw',1) \vdash (q_{0},w',b)$ donde $w_{1} = b_{i}$ y $w_{2} = w'$.
			
			Para el paso inductivo se sigue de manera idéntica.
			Supongamos que vale que: \newline			
			$(q_{0},w_{1}w_{2},1) \overset{*}{\vdash} (q, w_{2}, \pi(w_{1}))$ para $p\in \{ q_{0}, q_{1}\}$.
			Sea $w_{2}=b_{i}w_{2}'$ luego si $p,q \in \{ q_{0}, q_{1}\}$ y tenemos que $(q,w_{2},\pi(w_{1})) \overset{*}{\vdash} (p,w_{2}',\pi(w_{1}) \cdot b_{i})$.
		...
		Dada $w \in WP(F, B)$ tenemos que $(q_{0},w,1) \overset{*}{\vdash} (q_{1}, \epsilon, 1)$.
