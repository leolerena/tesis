...
\begin{equation*}
	L({\cal M}) = \{ w \in \Sigma^* \ | \ (q_0,w,Z_0) \vdash^* (q, \lambda, \epsilon), \ q \in Q, u \in \Sigma^*    \}
\end{equation*}
...
En cierta manera estamos diciendo que de un instante dado solo tenemos a lo sumo una única posibilidad de movernos a otro estado.
 ...
 Por lo tanto para cada palabra tenemos un único camino en el automáta para saber si es aceptada o no a diferencia de los \APND \  que podrían tener varios caminos posibles para cada palabra.
\begin{obs}
...
\begin{prop}
	Los lenguajes aceptados por automátas de pila determinísticos son cerrados por complemento.
\end{prop}

Valiendonos de esta proposición podemos ver que la contención de los lenguajes aceptados por autómatas de pila determinísticos dentro de los lenguajes aceptados por \APND es estricta. El ejemplo que nos sirve es el de los palíndromos que anteriormente vimos que un \APND lo aceptaba.

\begin{ej}
	El lenguaje $L = \{ w \in \{ 0,1 \}^*  \ : \ w = w^r \}$ no es aceptado por un \APD. Si lo fuera por la proposición anterior debería ser que $\overline L$ también es aceptada por un \APD.
	Notemos que en este caso 
	\begin{equation*}
		\overline L = \{  \}
	\end{equation*}
	que es un lenguaje que en particular no es siquiera aceptado por un \APND ya que usando el lema del pumping..
\end{ej}
