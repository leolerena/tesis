...
\begin{frame}[fragile]{Grafo de Cayley.}
	\defcajita{Sea $G$ grupo finitamente generado por $X$ diremos que el grafo de Cayley $\Gamma = \cay{G}{X}$ es el siguiente grafo no dirigido.
	\onslide<2->{
	\begin{align*}
		V(\Gamma) &= G   \\ 
		E(\Gamma) &= \{ \{ g,gx \}  \mid g \in G, x \in X  \}. 
	\end{align*}}}
...
\end{frame}
	...
	\begin{frame}[fragile]{Gramáticas (pt.2).}
		...
		\pause 
		Si $\gamma \tto \delta$ diremos que $\gamma$ \emph{deriva} en $\delta$.
		\pause 
	...
	\begin{frame}[fragile]{Teorema de Muller--Schupp}
		\defcajita{Decimos que un grupo $G$ es independiente de contexto si existe un (o equivalentemente, para todo) conjunto de generadores $A$ vale que el lenguaje $\WP{G}{A}$ es independiente de contexto. }
		\pause 
	...
	\begin{frame}[fragile]{Grafos como espacios métricos.}
		...
		\pause 
		\defcajita{Sea $\Gamma$ un grafo conexo luego dado un conjunto $C$ definimos el \emph{borde de vértices} de $C$ como
		\[
	...
	\begin{frame}[fragile]{Forma normal de Chomsky.}
		\defcajita{
			Una gramática $\gramatica$ independiente de contexto está en \emph{forma normal de Chomsky} si las producciones son de este tipo: \pause 
			\begin{enumerate}
	...
	\begin{frame}[fragile]{Lema 3.}
		...
		Sea $uvw \in L(\cG)$ de modo que $|v| \ge 2$ y una derivación $\deri = S \tto uvw$ \pause 
		...
		\]
			donde:
			\begin{itemize}
				\item $u'v'v''w' = uvw$. \pause
				% \item La palabra $v'v''$ es la palabra más chica que contiene a $v$ como subpalabra que deriva de una variable.
				\item $A \to BC \deriva v'v''$, $B \deriva v'$ y $C \deriva v''$. \pause
				\item $\delta \deriva u'$ y $\eta \deriva w'$. \pause
				\item $v$ está contenida en $v'v''$ como subpalabra y $v$ no está contenido en $v'$ ni en $v''$ como subpalabra. \pause
				\item $|u| \ge |u'|$ y $|v| \ge |v'|$. 
			\end{itemize} 
		}
	\end{frame}
	...
	\begin{frame}[fragile]{Teorema de Muller--Schupp.}
		\begin{columns}
			...
			\begin{column}{0.35 \textwidth}
				...
				Entonces: 
				\begin{align*}
					d(g,x) &= d(1,y) - d(1,x)  \le d(x,y) \\
					d(h,z) &= d(1,y) - d(1,z) \le d(z,y) \\ 
				\end{align*}
