...
\begin{frame}[fragile]{Grafo de Cayley.}
	\defcajita{Dado un grupo $G$ finitamente generado por $A$ podemos considerar un grafo $\Gamma =\cay{G}{A}$ que es el grafo de Cayley.
...
\end{frame}
	...
	\begin{frame}[fragile]{Lema 3.}
		Equivalentemente el lema anterior se puede escribir de esta manera.
	...
	\begin{frame}[fragile]{Teorema de Muller--Schupp.}
		\begin{columns}
			...
			\end{column}
			\begin{column}{0.35 \textwidth}
				Si consideramos el \emph{ciclo} $\alpha \tau \gamma$ luego las etiquetas del camino forman una palabra $uvw \in \WP{G}{X}$.
