...
De ahora en más consideraremos que $\Gamma$ es un grafo conexo y localmente finito salvo que se mencione lo contrario.
En todo momento vamos a pensar que el grafo $\Gamma$ es el grafo de Cayley de un grupo finitamente generado pero como estos resultados son válidos para grafos más generales enunciaremos los resultados de esta manera.
...
Si consideramos una bola $B$ de radio mayor a esta distancia entonces tenemos que $C \subseteq B$ o bien $\ol C \subseteq B$ dado que en caso contrario como el complemento de una bola es conexo tendríamos que habría algún $v' \in \Gamma \setminus B$ tal que $v' \in \beta C$ contradiciendo la elección de $B$.
Por lo tanto obtenemos que $|C| < \infty$ o bien que $|\ol C| < \infty$.
	...
	\end{figure}
\todo[]{Este dibujito no podría ser más feo... No es tan claro. Rehacer.}
...
Un dato importante es que en el contexto de los grupos la cantidad de ends de los grafos de Cayley resultan ser invariantes por presentaciones.
...
En esta sección vamos a introducir la definición de los grafos accesibles.
...
Un grafo es accesible si existe $k\in \NN$ tal que para todo par de ends $\omega_1, \omega_2$ resulta que existe un k-corte $C$ de manera que separa a ambos ends.
Equivalentemente podemos dar esta otra definición.
...
Esto nos dirá que va a estar bien definido en este contexto hablar de grupos accesibles dado que anteriormente probamos que tener treewidth finito es un invariante por cuasisometría.
...
Recordemos que todo grafo de Cayley de un \fg es tal que tiene grado acotado uniformemente por la cantidad de generadores.
...
\todo[inline]{Dibujo del árbol de estructura? Me parece haber visto que se podía hacer.}

Los árboles de estructura fueron introducidos y demostradas todas sus propiedades que describimos en esta sección en los trabajos de Dunwoody \cite{dunwoody1979accessibility} y \cite{dunwoody1982cutting}.
...
\end{proof}
\todo[]{Agregar un ejemplo de un árbol de estructura.}
\begin{center}
...
Esta acción se traslada naturalmente a una acción sobre el árbol de estructura tal como queríamos ver.

Como en la sección anterior sea $\Gamma$ un grafo accesible, conexo y localmente finito tal que $\aut(\Gamma)$ actúa con finitas órbitas sobre este grafo.

Probemos que $\aut(\Gamma)$ actúa sobre $\copt$.
...
\todo[inline]{Quizá comentar que esto nos da una manera de ver que un grafo qi con un árbol tiene tw finito.}
