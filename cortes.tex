...
\end{deff}	
\todo[inline]{Dibujito de un 2-corte.}
...
\todo[inline]{Ejemplo o dibujito de grafo accesible.}
...
%	
%	Esta suposición nos dice que en el caso que $D \in {\cal C}_{\text{min}}(\alpha)$ luego por ser un corte óptimo para $\beta$ resulta que $D \in \copt(\alpha)$.
...
%	Afirmamos que hay una esquina $K$ de los cortes $C$ y $D$ 
%	tal que $|K \cap \alpha| < \infty$ y tal que $|K \cap \beta| < \infty$.
...
%	es una componente conexa de $E$ tenemos que $\delta F \subseteq \delta E$ y similarmente para el caso de $F'$ dentro de $E'$.
...
\todo{Agregar lema que el maximal respecto a la inclusión está en la clase de equivalencia de $C$.}
...
\todo{Escribir interpretación de esta relación como contenciones y etc..}
\begin{prop}
...
%	
%	Para empezar sea $v \in \delta C \cap \ol C$ 
%	
%	
%	
%	y consideremos $n = \max  \{d(v,w) \ | \ w \in \delta C\}$.
%	Como $\ol C$ es conexo tenemos que $N^{n+1}(C) \cap \ol C$ resulta ser conexo.
%	Esto porque si $w,w' \in N^{n+1}(C) \cap \ol C$ luego su distancia a $C$ está acotada por $n+1$ por lo tanto existe un camino que lo une con $C$ menor a esa longitud.
%	Si ambos pasan por $u,u' \in \delta C \cap \ol C$ vértices que estén en el borde luego por nuestra elección de $n$ tenemos que $d(u,v) \le n$ y que $d(u',v) \le n$ concluyendo así que $N^{n+1}(C) \cap \ol C$ es conexo.
...
%	 $g \cdot C \sim C$ luego si $D \sim C$ obtenemos que dado que la acción de $G$ está bien definida sobre $T(\copt)$ que $g\cdot D \sim g\cdot C \sim C$ tal como queríamos ver.		
...
\begin{proof}
	...
	\todo{Comentar mejor como ir de sacarle S a $\ca$.}
