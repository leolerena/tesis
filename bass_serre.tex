...
\begin{proof}
	...
	de manera que $P_0 = P, P_k = P$, $P_i \in V(\iota (Y))$ siguiendo nuestra notación anterior y donde $e_i \in G_{s(y_i)}$ tal que $e_i t(y_{i+1}) = P_{i+1}$.
	
	Una primera observación que podemos hacer es que $e_k P = g P$ dado que resultan ser el mismo vértice.
	Esto nos dice que $g e_k^{-1} \in G_P$.
	Por el resultado anterior tenemos que $\varphi(h) = h $ para todo $h \in G_P$.
	Esto nos dice que al ser $\varphi$ un morfismo de grupos luego 
	\[
	\varphi(ge_k^{-1}) \varphi(e_k) = \varphi(g) 
	\]
	entonces como $ge_k^{-1} \in G_P \le \pi_1(\cG, P)$ luego si vemos que $\varphi(e_k )\in \pi_1(\cG, P)$ probaríamos que $\varphi (g) \in \pi_1(\cG,P)$ tal como queríamos ver.
		
	Para ver esto lo probaremos por inducción en el camino que tomamos anteriormente.
	Veamos que $e_i \in \Pi_1(\cG, P, P_i)$ para todo $i = 0 \dots n$.
	
		
\end{proof}

\begin{deff}
	Sean $\Gamma, \Gamma'$ grafos. Diremos que $\phi:\Gamma \to \Gamma'  $ es un morfismo de grafos si es una función   
	tal que:
	\begin{itemize}
		\item $\psi (V(\Gamma)) \subseteqq  (V(\Gamma'))$;
		\item $\psi (E(\Gamma)) \subseteqq  (E(\Gamma'))$;
		\item Si $y \in E(\Gamma)$ entonces $\ol{\phi(y)} = \phi(\ol y)$;
		\item Si $y \in E(\Gamma)$ entonces $\phi(s(y)) = s(\phi(y))$
	\end{itemize}   
\end{deff}
...
Dado que $gh^{-1} \in G_P$ luego tenemos que $\varphi(gh^{-1}) \in G_P$ porque $\varphi$ restringida a $G_P$ es la identidad por el resultado \ref{}.

\begin{lema}
	La función $\psi: \Xm \to X$ resulta ser un morfismo de grafos.
\end{lema}
\begin{proof}
	Esta claro por como está definido que cara vértice de $\Xm$ va a parar a un vértice de $X$ y que cada arista va a parar a una arista.
...
\begin{deff}
	Un morfismo de grafos $\psi:\Gamma \to \Gamma'$ se dice localmente inyectivo si para todo vértice $P \in V(\Gamma)$ resulta que $\psi: st(P) \to E(\Gamma')$ es inyectiva. 
\end{deff}
