...
\todo{Agregar esto en la sección que lo usamos.}
\begin{lema}\label{lema_union_arboles}
...
Primero vamos a dar una definición estándar que es la del cociente de un grafo no dirigido que va a ser la herramienta principal para probar el resultado central de esta sección.

\begin{deff}
	Sea $Y$ un grafo conexo y finito y sea $Y'$ un subgrafo conexo de $Y$ entonces
...
Si $T$ es un árbol generador de $Y$ entonces vamos a caracterizar los elementos del grupo $\pi_1(\cG, T)$ a partir de sucesiones reducidas, tal como hicimos en el caso particular de los productos amalgamados y las extensiones HNN.
...
\begin{deff}
	...
	\end{enumerate}
\end{deff}
En particular si tomamos una palabra $(c, \mu)$ tal que el camino $c$ no tiene backtracking: esto es que no existe $y_i$ tal que $\ol {y_{i+1}} = y_{i}$ entonces por la propiedad \textbf{R2} tenemos que esta palabra va a ser reducida.
...
\begin{center}
	\missingfigure[figwidth=6cm]{Dibujo para entender esta situación.}
\end{center}
...
\todo[]{En realidad alcanza con pedir que los grupos de los vértices sean \fg pero escribí la construcción usando que eran finitos. Se podría cambiar.}
\begin{deff}
...
\begin{proof}
	\todo{No me gusta como está escrita esta demo}
	...
	Probamos ahora que toda cadena $(S_i, \iota_i)_{i \in \NN} \in \Omega$ tiene un supremo en $\Omega$ por lo tanto por el lema de Zorn concluiríamos que existe un elemento maximal de $\Omega$.
	...
	Por esta definición tenemos que $\iota$ es una sección de $\pi$ y por el resultado \ref{lema_union_arboles} tenemos que $S$ y $\iota(S)$ resultan ser árboles.
	Tomamos como supremo para esta cadena a $(S, \iota) \in \Omega$.
...
En particular bajo esta definición obtenemos que el grafo de Cayley sigue siendo conexo y que $G$ sigue actuando de manera libremente sobre $X$ y más aún como $G$ actúa libremente sobre $\Gamma$ tenemos que la acción de $G$ en $X$ es sin inversiones de aristas.
