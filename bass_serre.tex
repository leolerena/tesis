...
\end{deff}
	...
	\begin{deff}
		...
		Una \emph{orientación} de las aristas de $Y$ es un subconjunto $A \subset E$ que cumple que para cada par de vértices unidos por una arista $y$ tenemos que $y \in A \iff \overline y \notin A$.	
	\end{deff}
...
\end{deff}	
	...
	En el caso particular que $P = Q$ permitimos tomar el camino vacío $c = ()$. 
...
Si notamos por $a$ a cada elemento de $G_y$ notaremos por $\overline{a}$ a la imagen de $a$ por el isomorfismo en el grupo $G_{\ol y}$.
...
En esta sección vamos a caracterizar los elementos del grupo $\pi_1(\cG, P)$ a partir de sucesiones reducidas tal como hicimos en el caso particular de los productos amalgamados y las extensiones HNN.

Primero que nada vamos a definir las sucesiones que nos van a interesar.
\begin{itemize}
...
En particular de este resultado se desprenden dos corolarios importantes para entender la estructura de los grupos fundamentales de grafos de grupos finitos.
...
\begin{proof}
	...
	Sea  $g \in G_{P} \setminus \{ 1 \}$ entonces $(c, \mu) = g$ donde $c = ()$ es el camino vacío y $\mu = (g)$.
...
\begin{coro}\label{coro_pal_red_2}
	Sea $\cG$ un grafo de grupos sobre un grafo finito y conexo $Y$, sea $c = y_0 \dots y_n$ un camino no vacío y $(c, \mu)$ una palabra reducida entonces $|c, \mu| \notin G_{s(y_0)}$.
\end{coro}
\begin{proof}
	Vamos a suponer que esto no es cierto y contradecir el teorema \ref{teo_pal_red}.
	Sea $(c, \mu) = r_0y_1r_1\dots r_n$.
	Si no fuera cierto tendríamos que $|c,\mu| = x \in G_P$. 
	Consideremos entonces otra palabra $(c,\mu')$ tal que $\mu' = (x^{-1}r_0, \dots, r_n)$ de manera que por como la tomamos $|c,\mu'| = 1$.
	Es una palabra reducida porque por suposición tenemos que $c$ no es el camino vacío y aparte $\mu'$ es idéntica a $\mu$ salvo en la primera posición que no afecta la condición \textbf{R2}.
