...
Llamemos a los estabilizadores $\pi_{\tilde P}$ y $\pi_{\tilde y} $ respectivamente para diferenciarlos. 
A su vez diferenciaremos por $\tilde P$ a la imagen de $1$ dentro de los cosets e idénticamente lo haremos para $\tilde y$. 
...
\begin{itemize}
	...
	Así tenemos usando nuevamente que $s(y) = t(\ol y)$ lo que queríamos probar.  
\end{itemize}

Finalmente la definición \textbf{(c)} es válida porque si usamos lo anteriormente demostrado y reemplazamos $y$ por $\ol y$ obtenemos este resultado.
...
\begin{proof}
	...
	\[
	\ol{s_{i+1} \tilde{y_{i+1}}} = s_{i+1} \tilde{\ol y_i} 
	\]
	pero esto es una contradicción porque al camino original $\tilde c$ lo habíamos tomado sin backtracking.
	
	Para concluir notemos que estamos la palabra $(c, \mu)$ cumple las hipótesis del resultado \ref{coro_pal_red_3} dado que es camino cerrado no trivial y es una palabra reducida por lo tanto $|c, \mu| \neq 1$ visto en $\pi_1(\cG, T)$ y esto contradice lo anteriormente obtenido.
\end{proof}
