...
La mayoría de las construcciones de esta sección siguen la exposición de \cite{serre2002trees} aunque la notación no es exactamente la misma sino que es similar a la utilizada en el trabajo \cite{diekert_contextfree_2017} 

\section{Definiciones básicas.}

	Un \emph{grafo dirigido} es un par ordenado $Y= (V(Y),E(Y))$ tal que:
	\begin{itemize}
		\item Al conjunto $V(Y)$ lo llamaremos los \emph{vértices}.
		\item Al conjunto $E(Y)$ las \emph{aristas}.
	\end{itemize}
	Para cada arista $y \in E(Y)$ tendremos una designación $s(y) = P \in V(Y), t(y) = Q \in V(Y)$.
	Informalmente diremos que $s(y)$ es el comienzo de la arista $y$ mientras que $t(y)$ es el final de la arista.
	A su vez para cada $y \in E(Y)$ tendremos $\overline y \in E(Y)$ la arista con \emph{asignación opuesta}.
	Esta arista cumple que $s(\ol y) = Q, t(\ol y) = P$. 
	
	Para esta sección nos va a interesar que los grafos que consideremos sean \emph{conexos}. 
	Esto es que dado dos vértices $P,Q \in V(Y)$ exista un camino entre ellos.
	Un camino va a ser una sucesión de aristas 
	\[
		(y_0, y_1, \dots y_k) \ \ \text{tal que} \ y_i \in E(Y)  \ \text{para} \ \ i = 0 \dots k
	\]
	de modo que $s(y_{i+1}) = t(y_{i})$ y tal que $s(y_0) = P $ y $t (y_k) = Q$.
	...
	Dado un grafo $Y$ conexo siempre podemos construirnos un árbol $T$ tal que $V(T) = V(Y)$ y $E(T) \subseteq E(Y)$. 
	A este árbol lo llamaremos un \emph{árbol de expansión} de $Y$.
...
\begin{deff}
	...
	\begin{itemize}
		\item $\phi (V(Y)) \subseteq  V(Y')$;
		\item $\phi (E(Y)) \subseteq  E(Y')$;
...
Las acciones que nos van a interesar en este capítulo las vamos a tomar para que \emph{no tengan inversiones} esto es que $g \cdot y \neq \ol y$ para toda arista $y \in E(Y)$ y para todo $g \in G$.
...
\begin{deff}
	...
	\begin{enumerate}
		...
		\item  Para cada arista $y \in E(Y)$ tenemos un grupo $G_y$ tal que $G_y$ es un subgrupo de $G_{s(y)}$.
...
En general para la definición de grafo de grupos no es necesario pedir para la condición \textbf{2} que haya una contención sino que alcanza con tomar un monomorfismo del grupo de la arista en el grupo del vértice del inicio de la arista. 
...
\begin{deff}
	...
	\begin{equation*}
		F({\cal G}) = (\underset{P \in V(Y)}{\Asterisk} G_P \Asterisk F_{E(Y)}) / \langle \langle  R \rangle \rangle
	\end{equation*}
	donde el conjunto de relaciones por las que dividimos es 
	\[
		R = \{  \overline y = y^{-1}, \ ya^yy^{-1} = a^{\overline y} \} \ \ \text{para toda $y \in E(Y)$ y todo \  $a \in G_y$}
	\]
\end{deff}

\begin{obs}
	El conjunto de relaciones $R$ por el que dividimos anteriormente se puede reescribir de una manera condensada como 
	\[
	R = \{  ya^y \overline y = a^{\overline y}  \} \ \ \text{para toda $y \in E(Y)$ y todo \  $a \in G_y$}.
	\]
\end{obs}
...
\begin{deff} \label{def_pi1_arbol}
	...
	\footnote{Como suponemos que $Y$ es conexo existen (posiblemente varios) árboles de expansión. Más en adelante probaremos que esta construcción no depende del árbol de expansión elegido.}
	Nos armamos el siguiente \emph{grupo fundamental del grafo de grupos}
	\begin{equation*}
		\pi_1({\cal G}, T) = F({\cal G}) / \{ y \  | \ y \in E(T)  \}.
	\end{equation*}
\end{deff}

Esto es que para todas las aristas que aparecen en el árbol de expansión las identificamos con la identidad en el grupo. 
...
Más aún es la generalización que tenemos del grupo fundamental de un complejo simplicial cuando dividimos por algún árbol de expansión.

\begin{obs}
	Notaremos por $g_{y}$ a la imagen de $y \in F(\cG)$ en el cociente $\pi_1(\cG,T)$.
\end{obs}
...
\begin{ej}
	...
	Esto es que $A$ es un subconjunto de la aristas que cumple que para cada par de vértices unido por una arista $y$ tenemos que $y \in A \iff \overline y \notin A$.
	...
	\]
	Si reescribimos esto recordando que el grupo fundamental de un grafo como espacio topólogico es el grupo libre en sus aristas que no pertenecen a un árbol de expansión nos queda que 
	\[
...
Nuestro foco ahora es dar otra definición de grupo fundamental pero esta vez usando caminos así como lo hacemos para el grupo fundamental usual de un espacio topológico.
...
\begin{deff}
	\emph{El grupo fundamental de $\cG$ respecto a un punto de base $P$} se define como $\pi_1(\cG, P) = \pi(\cG,P,P)$.
\end{deff}
...
Una \emph{forma normal para una extensión HNN} va a ser una sucesión $(x_{0},g^{\epsilon_0},x_{1},g^{\epsilon_1}, \dots, x_{n},g^{\epsilon_n},x_{n+1})$ donde $x_0 \in G_P$ es algún elemento arbitrario de este grupo.
...
En cambio en el paper \cite{diekert_contextfree_2017} construyen este árbol de una manera más sintáctica usando sistemas de reescritura.
