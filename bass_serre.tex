...
\begin{proof}
	...
	Sean $c' = w_1 \dots w_{n}$ y $\mu' = r'_0, \dots, r'_n$ queremos ver que si existe $1 \le k \le n$ tal que  $w_{k+1} = \ol{w_k}$ entonces $r_k \notin H_{w_k}$.	
	...
	\begin{enumerate}[i)]
		\item En el primer caso tenemos que  $w_{k} = y_i, w_{k+1} = \ol y_i$ para cierta $y_i$ que aparece en $c$, esto es que $s(y_i) \in V(Y) \setminus V(Y')$.
		Queremos ver que $r'_k \notin H_{\ol{w_k}}$.
		En este caso $H_{\ol{w_k}}$ es un subgrupo de $H_{[Y]}$ aunque resulta ser isomorfo a $G_{\ol{y_i}}$.
		Por ser $(c, \mu)$ reducida tenemos que $r'_k \notin G_{\ol{y_i}}$.
