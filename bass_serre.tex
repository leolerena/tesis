...
%Diremos que esta sucesión es \emph{reducida} .
%\begin{teo}[Britton]
%	Sea $G_P^{\ast G_y}$ una extensión HNN entonces 
%	para todo $g \in G_P^{\ast G_y}$ existe una única forma normal reducida	
%	$(x_{0},y^{\epsilon_0},x_{1},y^{\epsilon_1}, \dots, x_{n},y^{\epsilon_n},x_{n+1})$ 
%	tal que $g = x_{0}y^{\epsilon_0}x_{1}y^{\epsilon_1} \dots x_{n}y^{\epsilon_n}x_{n+1}$.
%\end{teo}
%\begin{proof}
%	Por el teorema \ref{prop_hnn_formanormal} tenemos que $g$ se puede escribir como el producto de una forma normal.
%	Basta observar que toda forma normal al reducirla sigue siendo una forma normal y que al reducirla se reduce la longitud de la sucesión por lo tanto este procedimiento eventualmente termina y así obtenemos una forma normal reducida.
%\end{proof}
%
%En particular nos va a resultar conveniente usar el siguiente corolario que se puede escribir como una negación del teorema anterior en el caso especial que $g=1$.
%
%\begin{coro}
%	Sea $G_P^{\ast G_y}$ una extensión HNN y
%	 $(x_{0},y^{\epsilon_0},x_{1},y^{\epsilon_1}, \dots, x_{n},y^{\epsilon_n},x_{n+1})$
%	una forma normal tal que si:
%	\begin{itemize}
%		\item $n=0$ entonces $x_{0}\neq 1$.
%		\item $n > 0$ entonces es una forma normal reducida. 
%	\end{itemize}
%	 entonces $x_{0}y^{\epsilon_0}x_{1}y^{\epsilon_1} \dots x_{n}y^{\epsilon_n}x_{n+1} \neq 1$ en $G_P^{\ast G_y}$.
%\end{coro}
...
%Similarmente al caso de las extensiones HNN vamos a definir una sucesión reducida.
%Que se corresponden a palabras en el grupo libre $G_{P} \ast G_{Q}$ tales que no tienen a ninguna relación como subpalabra.
%
%\begin{deff}
%	Sea $G_P \ast_{G_y} G_Q$ un producto amalgamado.
%	Una \emph{sucesión reducida} para el producto amalgamado es una sucesión
%	$(x_0,x'_0,\dots, x'_n)$
%	tal que para todo $0 \le i \le n$ tenemos que:
%	\begin{itemize}
%		\item $x_{i} \in G_{P}$ y que $x'_{i} \in G_{Q}$.
%		\item $x_{i}, x'_{i} \notin G_{y}$ para todo $0 < i < n$.
%	\end{itemize} 
%\end{deff}
...
Separamos en casos dependiendo cómo es el grafo $Y$.
	\begin{enumerate}
		...
		De esta manera aplicando el paso inductivo obtenemos que vale este resultado para el grafo de grupos $\cH$ \todo{Agregar cita a la definición del grafo de grupos sobre el grafo cociente? No sé como manejarme en estas demos.} 
...
\todo{Quizá podría recordar qué es $g_y$}
...
\todo[inline]{Hacer lo siguiente: 1) Escribir mejor esto del levantado, quizá agregar un dibujo. 2) Mostrar cómo es el árbol de Bass-Serre localmente. Su star está en biyección con los cosets de GP/Gy.}
...
\begin{obs}\label{obs_subgrafo_dom_fund}
	El subgrafo $W \subseteq \Xm$ generado por el siguiente conjunto de vértices \todo{definitivamente agregar definición de subgrafo generado por unos vértices}
\[
...
\begin{proof}
	...
	\begin{itemize}
		...
		\todo{Agregar que la unión de grafos es un grafo?}
		\[
		...
		para todo $g, h \in G$
		tenemos que $W \subseteq W_{g} \cap W_{h}$ con $W \neq \emptyset$ luego tendremos que $\Xm$ es conexo tal como queríamos ver.
		\todo{Agregar esta proposición de grafos conexos? Es un caso especial de la de conjunto conexos.}
		...
		\[
			s_{n} W \cup W
		\]
		es conexo por lo tanto si multiplicamos a izquierda por $s_{1} \dots s_{n-1}$ va a seguir siendo conexo:
...
\begin{deff}
	Sea $\cG$ un grafo de grupos sobre un grafo conexo $Y=(V,E)$.	
