...
\begin{frame}[fragile]{Merge.}
	...
	\pause
	\begin{codebox}
		\Procname{$\proc{merge}( \Co : \text{Tabla de cosets}, \alpha : \text{coset}, \beta : \text{coset}) : (\Co' : \text{Tabla de cosets})$}
		\li 
		\li 
	\end{codebox}
...
\begin{codebox}
	...
	\li $n=1, p[1]=1$,
	\li \textsc{ProcesarCoset}$(\Co, 1, Y)$ 
	\li \textsc{LoopOverCosets}$(\Co, A)$
	\End
\end{codebox}

\pause
Donde las funciones auxiliares son variaciones de \scanfill que realicen este procedimiento en listas de palabras y sobre todos los cosets definidos hasta el momento.
