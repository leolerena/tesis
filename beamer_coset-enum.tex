...
%		Al terminar de ejecutar el algoritmo \coin($\textbf{i}, \textbf{j}$)
%		\begin{enumerate}
%			\item $\Omega \simeq \In / \equiv$
%			\item Si antes teníamos $\textbf{k}^x = \textbf{k}$ entonces ahora tenemos que $\rep(\textbf{k})^x = \rep (\textbf{k})$.
