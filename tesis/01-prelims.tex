...
Consideremos un conjunto no vacío $\Sigma$, que llamaremos un \emph{alfabeto}.
Dado $k \in \NN$ denotaremos $\Sigma^k$ al conjunto de palabras en $\Sigma$ de exactamente $k$ elementos.
...
\begin{deff}
	Sea $\Sigma$ un alfabeto entonces el \emph{lenguaje de los palíndromos sobre $\Sigma$} es el siguiente lenguaje:
	\[
		L_{\text{PAL}} = \{  w \in \Sigma^* \mid w = w^R \}.
	\]
\end{deff}

En general cuando nos referiremos al lenguaje de los palíndromos asumiremos que el alfabeto es $\Sigma = \{a,b\}$.
...
\begin{deff}
	...
	\begin{itemize}
		...
		\item $P \subseteq ((V \cup \Sigma)^{*} \setminus \Sigma^{*}) \times (V \cup \Sigma)^*$ es un conjunto finito de \emph{producciones}.
...
La interpretación que le damos a una producción $\gamma \to_{\cG} \nu$ es que dada la palabra $\gamma$ la podemos reescribir por la palabra $\nu$.
Observar que la definición de producción garantiza que la palabra $\gamma$ es una palabra no vacía tal que al menos una de sus letras es una variable.
...
\begin{deff}
	Dada una gramática $\gramatica$ una variable $A \in V$ es \emph{alcanzable} si existe una derivación $\deri = S \deriva \delta A \eta$ para $\delta, \eta \in (V \cup \Sigma)^{*}$.
\end{deff}

\begin{prop}
	Para toda gramática $\gramatica$
	existe una gramática $\cG' =  (V', \Sigma, P', S)$ tal que todas sus variables son alcanzables, $V' \subseteq V$, $P' \subseteq P$ y $L(\cG) = L(\cG')$.
\end{prop}
\begin{proof}
	Sea el conjunto de variables
	\[
		I = \{ A \in V : A \ \text{no es alcanzable} \}.
	\]
	y sea el subconjunto de producciones
	\[
		P' = \{ (\gamma, \eta) \in P : \gamma, \eta \in ((V \setminus I) \cup \Sigma )^{*}  \}
	\] 
	Luego si $V' = V \setminus I$ podemos definir la gramática $\cG' = (V', \Sigma, P',S)$ tal que cumple lo pedido.
\end{proof}
...
\begin{ej}\label{gramatica-regular}
	...
	Sea $|w| = n$ entonces tenemos que la derivación es de la siguiente forma
	\[
		S \to aS \deriva aw'
	\]
...
\begin{deff}
	Sea $\gramatica$ una gramática independiente de contexto. 
	Para cada variable $A \in V$ consideramos el siguiente lenguaje
	\[
		L_{A} = \{  w \in \Sigma^{*} : A \deriva w \}.
	\]
	Si $L_{A} = \emptyset$ luego diremos que la variable $A$ es \emph{improductiva} y caso contrario diremos que $A$ es \emph{productiva}.
\end{deff}

En particular observemos que $L_{S} = L(\cG)$.

\begin{prop}
	Sea una gramática $\gramatica$ independiente de contexto, luego existe una gramática $\cG'$ tal que todas sus variables son productivas y 
	$L(\cG) = L(\cG')$.
\end{prop}
\begin{proof}
	Consideramos el subconjunto de variables
	\[
		I = \{ A \in V : L_{A} = \emptyset \}
	\]
\end{proof}
...
\begin{deff}
	...
	\end{equation*}
\end{deff}
Las \emph{configuraciones finales} para el lenguaje aceptado por estado final de un \APND van a ser las configuraciones que son de la forma $(q,\epsilon,\gamma)$ donde $q \in F$.
...
\begin{proof}
	...
	A los estados finales los distinguimos dibujándoles el borde dos veces.
...
Dicho de otra manera la configuración final para el lenguaje aceptado por pila vacía de un \APND es una configuración $(q, \epsilon, \epsilon)$.
...
%		Con esto probamos que $L$ es aceptado por un autómata de pila determinístico.
		...
		En este caso sea $m > |Q|$, luego si consideramos las palabras de la forma $\{ a^mb, \dots, a^mb^m  \} \subseteq L$ por el principio del palomar tienen que existir $1 \le i < j \le m$ tales que $(q_{0},a^mb^i, \$) \overset{*}{\vdash} (q,\epsilon,\gamma)$ y $(q_{0},a^mb^j, \$) \overset{*}{\vdash} (q,\epsilon,\gamma)$ para un mismo estado final $q \in F$.
...
\begin{deff}
	Si $k \in \NN_{\ge 1}$ definimos un \emph{k-autómata de pila finito determinístico} por medio de una tupla 
	\[
...
\begin{deff}
	Dado un conjunto $A$ podemos definir a $F_{A}$ el \emph{grupo libre} generado por los elementos de $A$ como un grupo que tiene una función  $\iota: A \to F_{A}$ inyectiva que denominamos la inclusión de los generadores en el grupo libre y que está definido por la siguiente propiedad universal: 
	...
	\begin{center}
		...
		\end{tikzcd}
	\end{center}
	En este caso diremos que $A$ genera libremente a $F_{A}$ y que $A$ es una \emph{base} de $F_{A}$.
\end{deff}

\begin{prop}
	Sean $F,F'$ grupos libres con respectivas bases $X, X'$.
	Entonces $F \simeq F'$ si y solo si $X$ y $X'$ tienen el mismo cardinal.
\end{prop}
\begin{proof}
	Ver \cite[p.13]{lyndon1977combinatorial}.
\end{proof}

\begin{deff}
	Dado $F$ grupo libre con base $X$ diremos que el cardinal $|X|$ es el \emph{rango de $F$}.
\end{deff}
	% Lo denotaremos $rg(F)$.
...
Dado un grafo no dirigido $\Gamma$ denotaremos por  $\Pi (\Gamma) = \{  c \mid c \ \text{es un camino en } \ \Gamma \}$ el conjunto de caminos en el grafo $\Gamma$.
...
\begin{lema}
	...
	Sea $\Gamma = (V,E)$ un grafo no dirigido. 
	Existe $\rho:V(\Gamma) \to \NN_{0}$ función tal que cumple las siguientes propiedades.
	\begin{enumerate}
	...
	\end{enumerate}
	si y solo sí $\Gamma$ es un árbol.
\end{lema}
\begin{proof}
	...
	\begin{itemize}
		\item \textbf{$\Gamma$ es conexo.}
			...
			El camino que tomamos es $(v, s(v), s^{2}(v), \dots, s^{\rho(v)}(v))$.
			Tal que está bien definido porque $\{v,s(v)\}$ es una arista para todo $v \in V \setminus \{v_{0}\}$ y como $\rho(s(v)) = \rho(v) - 1$ entonces necesariamente $s^{\rho(v)}(v) = v_{0}$.
	...
	Para la vuelta nos basta notar que si $\Gamma$ es un árbol luego podemos tomar $v_{0} \in V(T)$ dado que todo árbol es un grafo no vacío. 
	La función que definimos es $\rho(v) = d(v_{0},v)$ de manera tal que va a cumplir las tres propiedades.
...
\begin{obs}
	Esta definición que tomamos de grafo de Cayley no es la usual.
	Por ejemplo en \cite{bridson2013metric} los grafos de Cayley son grafos dirigidos en vez de ser grafos no dirigidos.
\end{obs}
...
%	y sea $\pi:(A \cup A^{-1})^* \to G$ la proyección de monoides entonces vale lo siguiente:
	\begin{enumerate}
		\item Si $c$ es un camino cerrado luego $l(c) = 1$ en $G$.
...
\begin{proof}
	...
	Esto nos dice que 
	$g_{0} a_{0} \dots a_{n-1} g_{0}^{-1}  = 1$ lo que implica que $a_{0} \dots a_{n-1} = 1$ , esto es que $l(c) = 1$ en $G$ tal como queríamos ver.
...
En particular si $G$ es un grupo finitamente generado por $A$ luego si consideramos esta misma distancia sobre $\cay{G}{A}$ entonces coincide con la siguiente distancia sobre el grupo $G$ (dado que el conjunto de vértices del grafo de Cayley es exactamente el grupo $G$).
Si $g,h \in G$ luego definimos
% Dado un grupo $G$ finitamente generado por $A$ podremos ver a $G$ como un espacio métrico con la siguiente métrica
\[
	d(g,h) = \min \ \{ |w| : w \in (A \cup A^{-1})^{*}, \ gw=h\}.
\]


Para finalizar notamos las siguientes dos observaciones:
\begin{enumerate}
...
\begin{deff}
	...
	\begin{itemize}
		...
		\item Dado un grafo dirigido $Y=(V,E)$ con funciones $s,t, \ol{(.)}$ diremos que $Y'=(V',E')$ con funciones $s',t'$ es un \emph{subgrafo} de $Y$ si $V' \subseteq V$, $E' \subseteq E$ y si $s'=\left. s \right|_{E'}, t' = \left. s \right|_{E'}$ 
...
Dada una sucesión de producciones de la siguiente forma:
\[
	{\cal D} =  A \deriva u
\]
