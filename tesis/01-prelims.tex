...
Consideremos un conjunto no vacío $\Sigma$, que llamaremos un \emph{alfabeto}, luego dado $k \in \NN$ denotaremos $\Sigma^k$ al conjunto de palabras en $\Sigma$ de exactamente $k$ elementos.
...
\begin{prop}
	Sea una gramática $\gramatica$ luego existe una gramática $\cG'$ tal que todas sus variables son alcanzables y $L(\cG) = L(\cG')$.
\end{prop}
\begin{proof}
	...
	\[
		P_{I} = \{ (\gamma, \eta) \in P : \gamma, \eta \in ((V \setminus I) \cup \Sigma )^{*}  \}
	\] 
	Luego $\cG' = (V \setminus I, \Sigma, P_{I},S)$ cumple lo pedido.
\end{proof}
...
\begin{deff}
	Sea un gramática independiente de contexto $\gramatica$ luego para toda variable $A \in V$ consideramos el siguiente lenguaje
	\[
