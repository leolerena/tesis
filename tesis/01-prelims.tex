...
\begin{prop}
	Sea una gramática $\gramatica$, luego existe una gramática $\cG' =  (V', \Sigma, P', S)$ tal que todas sus variables son alcanzables, $V' \subseteq V$, $P' \subseteq P$ y $L(\cG) = L(\cG')$.
\end{prop}
\begin{proof}
...
\begin{prop}
	Sean $F,F'$ grupos libres con bases $X, X'$.
...
\begin{deff}
	Dado $F$ grupo libre con base $X$ diremos que el cardinal $|X|$ es el rango de $F$.
\end{deff}
	% Lo denotaremos $rg(F)$.
