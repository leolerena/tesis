...
\begin{proof}
	...
	Tenemos que decir quien es $\delta'( (q,i), a, (z,j) )$. 
	Si $\delta(q,a,z) = \emptyset$, entonces definimos $\delta'( (q,i), a, (z,j) ) = \emptyset$.
	Si $\delta(q,a,z) = {(p,t)}$, con $t = t_k ... t_2 t_0$, vamos a definir recursivamente una sucesión $s_0, s_2, ... s_k \in A$.
	$s_0 = j$,
	$s_{l+1} = s_{l}+1 \ \text{si} \ s_l \neq \bot \ \text{y} \ s_{l+1} \le n \ \text{y} \ t_l = \gamma_{j+l} y s_{l+1} = \bot \ \text{en caso contrario}$.
	Definimos $t' = (t_k, s_k)...(t_1,s_1)(t_0,s_0)$ y sea $i' = 1 \ \text{si} \ s_k=n \ \text{y} \ t_k=\gamma_n, \ \text{y} \ 0 \ \text{en caso contrario}$.
	Finalmente $\delta'((q,i), a, (z,j)) = {((p,i'),t')}$.


	Sea ${\cal{M'}} = (Q', \Sigma, \Gamma', \delta', (q_{0},0), F', (\$,0))$.
