...
\end{prop}
\begin{proof}
	\TODO{Creo que esta demo puede verse como un caso de una demo de "simulación", voy a ver si encuentro una manera más compacta de escribirla.}
	...
	Si $w \notin L({\cal M}, \gamma)$ entonces $(q_{0}, w, \$) \overset{*}{\vdash} (q, u, \beta_k \dots \beta_0)$ donde necesariamente una de estas tres opciones ocurren: $u \neq \epsilon$, $q \notin F$ o $\beta_{k} \dots \beta_{0} \neq \gamma_{n-1} \dots \gamma_{0}$.
	En estos tres casos podemos ver que esto implica que $w \notin L({\cal M}')$. 
	\TODO{Acá no estoy siendo explícito pero sale directo usando que son autómatas determinísticos y de la afirmación. Si es necesario escribo los tres casos.}
...
\begin{deff}
	...
	\begin{itemize}
		...
		\item $\Sigma$ es un conjunto finito que denominamos el \emph{alfabeto del lenguaje};
		\item $\Gamma$ es un conjunto finito que denominamos el \emph{alfabeto de la pila};
		\item $\delta$ es la \emph{función de transición} donde $\delta: Q  \times (\Sigma \cup \{ \epsilon \}) \times \Gamma^{\le k} \to {\cal P}_( Q  \times \Gamma^*)$
...
\end{prop}
\begin{proof}
	\TODO{Revisar demo y refactorizar.}
	Si existe ${\cal M} = (Q, \Sigma, \Gamma, \delta_{\cal M}, q_0, F, \$)$ \APND tal que $L({\cal M}) = L$ luego $\cal M$ es un $k$-\APD por lo tanto nos basta con tomar ${\cal N}={\cal M}$. 
	...
	Si existe ${\cal N} = (Q, \Sigma, \Gamma, \delta_{\cal N}, q_0, F, \$)$ k-\APND tal que $L({\cal N}) = L$ luego sean:
	...
	\[ 
		...
		\implies
		((q,\epsilon,\epsilon), aw, \gamma_{1}\gamma_{2}\dots \gamma_{i}\alpha) \underset{\cal M}{\vdash^{*}} 
		((p,\epsilon,\epsilon), w, \beta \alpha) 	
	\]	
	\end{aff}
	\TODO{Escribir esta demostración.}
	...
	(p,u, \gamma)$ para ciertos $p \in Q$, $\gamma \in \Gamma^{*}$ y $u \in \Sigma^{*}$.
	Por la afirmación obtenemos que $((q_{0},\epsilon, \epsilon),w,\$) \overset{*}{\vdash} ((p,\epsilon,\epsilon),u,\gamma)$ por lo tanto $w \notin L({\cal M})$.
