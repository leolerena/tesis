...
Si $\Gamma = (V,E)$ es un grafo no dirigido simple luego podemos considerar $X$ un grafo dirigido tal que lo represente.
Lo definimos de manera tal que $V(X) = V$ y si $e = \{x,x'\} \in E$ luego tenemos dos aristas $e,d \in E(X)$ definidas de manera tal que $s(e) = x, t(e) = y$ y $\ol{e} = d$.


Si $Y$ es un grafo dirigido tal que no tiene bucles y tal que dados dos vértices distintos existe una única arista que los une (salvo orientación) entonces podemos considerar a $\Gamma$ un grafo no dirigido tal que representa a $Y$ de la siguiente manera:
\[
...
Por lo visto en el ejemplo \ref{ej_pi1_bucle} tenemos que dado $Y = (\{P\}, \{y,\ol y\})$ un bucle, entonces toda estructura de grafo de grupos $\cG$ que le demos a este segmento 
...
\begin{prop}[Lema de Britton]\label{teo_britton}
...
% Si elegimos conjuntos transversales de $G_{y}$ y de $G_{\ol y}$ nos podemos  
% quedar con algunas de las sucesiones reducidas.
% Estas sucesiones reducidas las llamaremos formas normales.
...
\begin{center}
	\begin{tikzpicture}[
		...
		\end{scope}	
		\draw   
		(1)  edge[->,bend right=45, "$y_1$"] (2)
		(2)  edge[->,"$y_7$", loop]  (2)
		(2)  edge[->,"$y_6$", bend right = 45] (1);
...
\begin{ej}
	...
	En este caso el subgrafo conexo $Y'$ es el que está pintado de color \textcolor{carrotorange}{naranja}.
	...
	\begin{center}
		\begin{tikzpicture}[
			...
			\end{scope}
			\draw   
					(1)  edge[->,line width=1.3pt,"$y_1$", bend right = 45] (2)
					(2)  edge[->,"$y_6$", bend right = 45] (1)
					(2)  edge[->,"$y_9$", loop]  (2)
...
\begin{enumerate}
			...
			\begin{center}
				...
				\begin{scope}[nodes=V]
					\node (1)  [minimum size=1.2cm] {$P$};
					\node (2) [right=of 1,minimum size=1.2cm]    {$Q$};
				\end{scope}
				\draw   (1)  edge[->,"$y$"] (2);
			\end{tikzpicture}
			\end{center}
...
$Y = \Xm / G$ entonces valdría lo siguiente:
para todo vértice $\widetilde{ P} \in V(\Xm)$ por la relación entre las órbitas y los estabilizadores existe una biyección entre la órbita $G \widetilde{P} $ y el conjunto de los cosets del estabilizador $G / G_{\widetilde P}$.
...
Por como definimos a los grafos de grupos tenemos que $G_{\ol y}$ es un subgrupo de $G_{s(\ol y)}$ y como $s(\ol y) = t(y)$ terminamos de probar que $g^{-1}_y h^{-1}g g_{y} \in G_{t(y)}$ tal como queríamos ver.
...
% 	Sea $X$ un grafo conexo y sean $\{X_{i}\}_{i \in \NN}$ subgrafos conexos de manera que para todo $i \in \NN$ vale que	
% 	$X_{i} \cap X_{i+1} \neq \emptyset$ 
% 	entonces $\bigcup_{i \in \NN} X_{i}$ es un subgrafo conexo de $X$.
% \end{lema}
...
\begin{ej}
	...
	Definimos una acción $\psi: G \to \textrm{Aut}(X)$, 
	de manera que $\psi(a)$ es un automorfismo idempotente definido de la siguiente manera:
	$\psi(a)(P_1) = P_5,
	\psi(a)(P_2) = P_4, 
	\psi(a)(P_3) = P_3, 
	\psi(a)(P_4) = P_2$ y  
	$\psi(a)(P_5) = P_1$.
	Sobre las aristas queda definido de la siguiente manera 
	$\psi(a)(y_{1}) = \ol{y_{4}},
	\psi(a)(y_{2}) = \ol{y_{3}},
	\psi(a)(y_{3}) = \ol{y_{2}}$ y 
	$\psi(a)(y_{4}) = \ol{y_{1}}.$
	Chequeamos que $(\psi(a))^{2} = I$ por lo tanto la acción está bien definida.

	Gráficamente representamos al automorfismo de la siguiente manera.
	\begin{center}
		\begin{tikzpicture}[
			node distance = 10mm and 10mm, 
			V/.style = {circle, draw, fill=gray!15,font=\footnotesize},
	...
	Más adelante probaremos que no importa qué sección tomemos el grafo de grupos que vamos a construir va a tener el mismo grupo fundamental.
\end{ej}
...
\begin{proof}
	...
	Como $ \varphi(ge_{k}^{-1}) = ge_{k}^{-1}$ obtenemos que  $ge_{k}^{-1} \subseteq \varphi(\pi_1(\cG, P))$.  
...
\begin{proof}
	...
	\begin{itemize}
		...
		\item Probamos ahora que toda cadena $(S_i, \iota_i)_{i \in I} \in \Omega$ tiene una cota superior en $\Omega$ por lo tanto por el lema de Zorn concluiríamos que existe un elemento maximal de $\Omega$.
		...
		\[
		\iota = \bigcup_{i \in I} \iota_i
		\]
		donde $\iota(y) = g \cdot y$ si existe $i \in I$ tal que $y \in S_{i}$ y en ese caso $\iota_{i}(y) = g\cdot y$.
		...
		Una cota superior para esta cadena resulta ser $(S, \iota) \in \Omega$.
	\end{itemize}
	...
	Si consideramos $M' =M \cup \{y\}$ luego $(M', j')$ con $j'(y) = g \cdot y$ y $j'(z) = j(z)$ para todo $z \in E(M)$ es tal que $(M',j') \in \Omega$ y $(M,j) < (M',j')$ contradiciendo la maximalidad de $(M,j)$.
