...
\begin{deff}
	...
	$s:E \to V$, $t:E \to V$ y $\ol{(.)}:E \to E$ tales para toda $y \in E$ vale que $s(\ol{y}) = t(y)$, que $\ol{\ol y} = y$ y que $\ol{y} \neq y$.
...
\begin{obs}\label{obs_subgrafo_dom_fund}
	El subgrafo $W \subseteq \Xm$ generado por el siguiente conjunto de vértices
		\[
		C = \{ G_{P} \mid \forall P \in V(Y) \} \cup \{ g_{y}G_{Q} : \forall y \in E(Y), Q \in V(Y) / \ t(y) = Q  \}
		\]
	es tal que $G \cdot W = \Xm$.
...
\begin{proof}
	...
	\begin{itemize}
		...
		El caso que $n=0,1$ es claro porque no existen caminos con estas longitudes de manera que no tengan backtracking.
...
\begin{teo}
	...
	Dado $\cG$ un grafo de grupos finito sobre un grafo conexo $Y$ y dado $T$ árbol generador de $Y$.
...
\begin{proof}
	...
	Por el resultado \ref{teo:subgrupo_normal_pi1_libre} tenemos que $F$ es un subgrupo libre de $\pi_1(\cG, P)$, que es un grupo finitamente generado, y queremos ver que $F$ tiene índice finito para probar que $\pi_1(\cG,P)$ es virtualmente libre.
