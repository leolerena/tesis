...
\begin{proof}
	...
	% 		Si $v \in f(t)$ entonces $v \in T_{v}(f)$ y por otro lado si $v \in N(f(t)) \setminus f(t)$ luego $v$
...
\TODO{Comentarle a Iván: Me parece que su propuesta para esta demo no funciona.
Creo que no vale decir que $T_{v}(g) =  \bigcup_{w \in N(\{v\})} T_{w}(f).$ porque no vale la contención del lado izq. en el lado der.}
...
\begin{obs}
	...
	Sea $\Gamma$ grafo no dirigido tal que $C \subseteq V(\Gamma)$ luego si  $C = \bigcup_{i \in I} C_{i}$ donde $C_{i} \cap C_{j} = \emptyset$ para todo par $(i,j) \in I \times I$ tales que $i \neq j$ entonces $\beta C = \bigcup_{i \in I} \beta(C_{i})$.
\end{obs}
...
\begin{deff}
	...
	\[
	V(T) = \{  \beta C : \exists l \in \NN / C \subseteq V_l \ \text{componente conexa} \}.
	\]
	...
	\[
		f(\beta C) =
		\begin{cases}
			\beta C & \text{si $\beta C \neq \emptyset$} \\
			\{1\}       & \text{si $\beta C = \emptyset$   } \\
		\end{cases}
	\]
