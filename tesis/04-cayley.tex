...
% En el caso que $\Gamma$ no es conexo podemos adaptar esta demostración tomando un árbol con tantos vértices como componentes conexas tenga $\Gamma$.
...
%	\]
%	
%	Sea $(T,f)$ una descomposición en un árbol para $\Gamma_{1}$ tal que tiene anchura finito.
...
%	\end{align*}
%	
%	Afirmamos que $(T,h)$ es una descomposición en un árbol para $\Gamma_{2}$ tal que tiene anchura finito.
%	
%	
...
%	\end{enumerate}
%	
%	Por el resultado \ref{coro_vecinos_anchura_grado} tenemos que $(T,g)$ tiene anchura finito dado que $(T,f)$ lo tiene. 
%	De esta manera también concluimos que $(T,h)$ tiene anchura finito por lo que $ \Gamma_{2} $ tiene treewidth finito tal como queríamos ver.
%\end{proof}
...
La demostración que presentamos sigue la exposición del trabajo \cite{diekert2017context}.
	...
	\begin{lema}\label{lema:gram_ic_chomsky_variable_chica_deriva}
		...
		% 	\[
		% 		\deri = S \deriva \delta A \eta \deriva u'v'w'
		% 	\]
		% 	para $\delta, \eta \in (V \cup \Sigma)^{*}$, $u',w' \in \Sigma^{*}$ tales que: $\delta \deriva u', A \deriva u', \eta \deriva w'$ y  $u'v'w' = uvw$.
		% \end{itemize}
	\end{lema}
	\begin{proof}
