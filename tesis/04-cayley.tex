...
\begin{leoenv}
	...
	\begin{lema}\label{lema:gram_ic_chomsky_variable_chica_deriva}
		...
		Sea $\deri = S \deriva uvw$ una derivación en esta gramática.
		Para cada subpalabra $v \in \Sigma^{*}$ tal que $|v| \ge 1$ se tiene que existe una variable $A \in V$ y una subpalabra $v'$ de $uvw$ minimal respecto a la longitud tal que:
		\begin{itemize}
			\item $v$ es subpalabra de $v'$;
			\item La derivación $\deri$ resulta ser igual a 
			\[
				\deri = S \deriva \gamma A \alpha \deriva u'v'w'
			\]
			para $\gamma, \alpha \in (V \cup \Sigma)^{*}$, $u',w' \in \Sigma^{*}$ tales que: $\gamma \deriva u', A \deriva u', \alpha \deriva w'$ y  $u'v'w' = uvw$.
		\end{itemize}
	\end{lema}
	\begin{proof}
		Para probar esto consideramos $T(\deri)$ el árbol de derivación asociado.
		La ocurrencia de la palabra $v = a_{1} \dots a_{n}$ es tal que está representada por un conjunto finito de vértices del árbol $V(T(\deri))$ (de hecho resultan ser hojas del árbol por la observación \ref{obs:arbol_deri_datos}).
		Sea este conjunto $V_{u}  = \{ (d_{1},l(d_{1})), \dots, (d_{n},l(d_{n})) \}$ donde $l(d_{i}) = a_{i}$.

		En este caso notemos que el problema se traduce a encontrar un vértice $(d,l(d)) \in V(T(\deri))$ tal que produzca a $v$ como subpalabra.
		Para esto notemos que aplicando el lema \ref{lema:arbol_binario_meet} al conjunto de hojas $V_{u}$ obtenemos un vértice $(d,l(d))$ interno tal que es el menor vértice con respecto al orden $\preceq$ que produce una palabra $v' \in \Sigma^{*}$ de manera que contiene como subpalabra a la palabra asociada al conjunto $V_{u}$.
		Como es un vértice interno luego $l(d) = A \in V$ por la observación \ref{obs:arbol_deri_datos} y a su vez resulta ser que no puede haber una palabra de longitud menor tal que cumpla esta propiedad caso contrario negaríamos la minimalidad del vértice $(d,l(d))$.
...
\begin{proof}
	...
	\begin{equation*}
		S \deriva \gamma A \alpha \to_{\cal G} \gamma BC \alpha \deriva u'v'v''w'
	\end{equation*}
	donde: $\gamma, \alpha \in (\Sigma \cup V)^{*},B,C \in V$ y $\gamma \deriva u', \alpha \deriva w', A \deriva v'v''$, $B \deriva v'$ y $C \deriva v''$.
...
\begin{tikzpicture}[font=\sffamily]
		...
		\draw[thick,path picture={
			\foreach \X in {A,B,C}
			{\draw[line width=0.4pt] (\X) circle (0);}}] (A) node[left]{$g$} to[bend left=20] node (y) [draw,circle,fill = black,pos=0.82,minimum size=.1cm, inner sep=0pt,label= above:y] {}
		...
		\node[label=right:{\Large{$\tau$}}] at (2.4,3.65) {};
