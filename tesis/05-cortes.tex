...
\begin{deff}
	...
	\begin{itemize}
		\item $C$ y $\olc C$ son conexos y no vacíos.
		\item $|\delta C| < \infty$.
	\end{itemize}
...
\begin{deff}
	...
	\[
	{\cal C}(\alpha) = \{ C \subset V(\Gamma) \mid  C \ \text{es un corte y} \ |\alpha \cap C| = \infty = |\alpha \cap \olc C| \}
	\] 
\end{deff}
...
\begin{proof}
	...
	Consideremos $D$ la componente conexa de $v_0$ en $\olc{ C_{t_l, r}}$.
...
%		Vamos a ver de acotar $|\beta D \cap D|$ y $|\beta D \cap \olc D|$.
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[scale=0.85]
		...
		\node (3) at (0.5,4.25)  {$\olc C \cap D$};
		\node (4) at (0.5+8,4.25)  {$\olc C \cap D$};
		\node (5) at (0.5,-0.25)  {$\olc C \cap \olc D$};
		\node (6) at (0.5+8,-0.25)  {$\olc C \cap \olc D$};
		\node (7) at (3.5,-0.25)  {$C \cap \olc D$};
		\node (8) at (3.5+8,-0.25)  {$ C \cap \olc D$};
	...
	\end{tikzpicture}
	\caption{\small{En la izquierda tenemos que si tomamos $E = \olc C \cap D$ y $E' = C \cap \olc D$ estas esquinas cumplen que $|E \cap \alpha| = \infty$ y que $|E' \cap \beta | = \infty$.
			En la derecha tenemos que nos alcanza con tomar $E = \olc C \cap \olc D$ y $E' = C \cap D$.
...
\begin{proof}
	...
	%	Si no están en un mismo corte, como podría ser que $v \in C \cap \olc D$ y $w \in \olc C \cap D$ luego usando la conexión de $\olc D$ y de $\olc C$ podemos unir ambos con algún vértice en $\olc C \cap \olc D$ y así vemos que es conexo.
