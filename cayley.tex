...
\begin{tikzpicture}[font=\sffamily]
	...
	\draw[thick,path picture={
		\foreach \X in {A,B,C}
		{\draw[line width=0.4pt] (\X) circle (0);}}] (A) node[left]{$g$} to[bend right=10] node (y) [draw,circle,fill = black,pos=0.62,minimum size=.1cm, inner sep=0pt,label= above:y] {}
	(B) node[above right]{$h$} to[bend right=25] node (z) [draw,circle,fill = black,pos=0.62,minimum size=.1cm, inner sep=0pt,label= right:z] {}
	...
	\node[label=left:A] at (barycentric cs:x=1,y=1) {};
	\node[label=right:B] at (barycentric cs:y=1,z=1) {};
	\node[label=below:C] at (barycentric cs:x=1,z=1) {};
\end{tikzpicture}
	
	\todo{Cambiar las variables... Están en el orden equivocado.}
