...
Dado $t \in V(T)$ diremos que $f(t) \in 2^{V(\Gamma)}$ es un \emph{bolsón}.
...
\begin{prop}\label{prop-camino-desc}
	...
	Sean $t_{1},t_{2},t_{3} \in V(T)$ tales que la única geodésica que une a $[t_1,t_2]$ pasa por $t_{3}$.
...
\begin{proof}	
	...
	Para el paso inductivo supongamos que vale para caminos de longitud $n-1$.
	Sea $t' \in V(T)$ de manera que $v_{0}, v_{1} \in f(t')$ que existe por la propiedad \textbf{T2}.
	Notemos que $t_{3}$ está en la geodésica $[t',t_{2}]$ dado que 
	$[t_{1}, t_{2}] = [t_{1}, t']*[t',t_{2}]$
	luego $t_{3}$ no está en la geodésica $[t_{1},t']$ porque no es ninguno de los dos vértices que aparece lo que obliga a que $t_{3}$ esté en la geodésica $[t',t_{2}]$.
	
	
	Entonces si $t' = t_{3}$ ya está porque nos alcanza con tomar $i=1$ de manera que $v_1 \in f(t_{3})$.
	En el caso que esto no ocurra miramos el camino de longitud $n-1$ dado por $(v_1 v_2 \dots v_n)$ y usando la hipótesis inductiva llegamos al resultado.	
\end{proof}
