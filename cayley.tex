...
\begin{ej}
	...
	Sea $\Gamma$ el siguiente grafo con descomposición en un árbol $T$ y 
	...
	\begin{scope}[nodes=V, xshift=-3.5cm, yshift=0cm]
		\node (1) [fill = brightcerulean] {$t_1$};
		\node (2) [below=of 1, fill=cadmiumred] {$t_{2}$};
		\node (3) [below=of 2, fill=brightube] {$t_3$};
	\end{scope}	
	\draw   (1)  edge[-,] (2)
	(2)  edge[-] (3);
		...
		\begin{scope}[nodes=V,xshift=4.5cm, yshift=0cm]
			\node (1)  [fill = brightcerulean] {$v_{1}$};
			\node (2) [below=of 1, fill = brightcerulean, fill fraction=cadmiumred]{$v_{2}$};
			\node (3) [right=of 2, fill = brightcerulean, fill fraction=cadmiumred	]    {$v_{3}$};
			\node (4) [below=of 2,fill = cadmiumred, fill fraction=brightube]    {$v_{4}$};
			\node (5) [below=of 3, fill = cadmiumred]    {$v_{5}$};
			\node (6) [below=of 4,fill = brightube]    {$v_{6}$};
		\end{scope}
			\draw   (1)  edge[-] (2);
			\draw   (1)  edge[-] (3);
			\draw   (2)  edge[-] (3);
