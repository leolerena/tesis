...
\begin{ej}
	Sea $\Gamma$ el grafo dado por $V = \{ v_{1}, v_{2},v_{3}, v_{4},v_{5},v_{6} \}$ con aristas \linebreak 
	$ E = \{ \{ v_{1}, v_{2}\}, \{ v_{1},v_{3} \},\{v_{2} ,v_{3} \} , \{ v_{2} , v_{5} \}, \{ v_{3} , v_{4} \}, \{ v_{5}, v_{4} \}, \{ v_{4}, v_{6} \} \} $.
	Entonces una descomposición está dada por $T$ árbol con vértices  $V(T)=\{ t_{1}, t_{2}, t_{3}  \}$ y con aristas $E(T) = \{ \{t_{1}, t_{2} \}, \{ t_{2}, t_{3} \} \}$ y un mapa 
	...
	\begin{center}
		\begin{tikzpicture}[V/.style = {circle, draw, fill=gray!15,font=\footnotesize},fill fraction/.style={path picture={
					\fill[#1] 
...
\begin{ej}
	...
	Este grupo es generado por el conjunto simétrico de generadores $\{ a, b, b^{-1} \}$ tal que al grafo $\text{Cay}(G, \{a,b,b^2\})$ lo representamos de la siguiente manera:
...
Consideremos entonces cómo es la descomposición en un árbol construida en \ref{desc-grafo-cayley}.
El árbol $T$ lo representamos de la siguiente manera tal que cada vértice $t \in V(T)$ es el bolsón correspondiente para la descomposición en un árbol de $\text{Cay}(G,\{ a,b,b^2 \})$.
   ...
   \begin{tikzpicture}
   	...
   	\node[align=center] at (7, 2) {Árbol \textbf{$T$}};
   	\begin{scope}[nodes=V,xshift=4.5cm, yshift=-4cm]
...
\begin{prop}\label{prop_qi_simetrica}
	Sean $(X,d_X),(Y,d_Y)$ espacios métricos. 
	Una función $f:X \to Y$ es una cuasisometría si y solo sí existe 
	$g:Y \to X$ cuasisometría y constantes positivas $C,D \in \RR$ tales que:
	\begin{itemize}
...
\begin{prop}\label{prop_qi_transitiva}
	Sean $(X,d_X),(Y,d_Y), (Z, d_{Z})$ espacios métricos y sean $f:X \to Y$ y $g:Y \to Z$ cuasisometrías,
	 entonces $g \circ f: X \to Z$ es una cuasisometría.
\end{prop}
\begin{proof}
	Ver \cite[p.86]{loh2017geometric}.
	\todo[]{La demo está como ejercicio en este libro, la podría escribir sino ja}
\end{proof}


\begin{deff}
	Dados $(X,d_{X})$ y $(Y,d_{Y})$ espacios métricos de manera que existe $f:X \to Y$ cuasisometría diremos que son espacios métricos \emph{cuasisométricos} y denotaremos $X \overset{q.i}{\sim} Y$ la relación de cuasisometría entre espacios métricos.
\end{deff}
...
\begin{prop} \label{treewidth-inv}
	Sean $(\Gamma_{1},d_{1}), (\Gamma_{2}, d_{2})$ grafos no dirigidos de grado uniformemente acotado cuasisométricos entre sí y $\Gamma_{1}$ grafo con treewidth finito.
...
\begin{deff}
	Un \emph{grupo $G$ finitamente generado tiene treewidth finito} si para algún conjunto generadores $A$ vale que $\text{Cay}(G,A)$ tiene treewidth finito.
\end{deff}
...
\begin{proof}
	...
	\] 
	para todo $t$ 
	entonces al ser $\Gamma$ de grado acotado uniformemente por $n$ luego 
	\[
		|f(t)| \le n^{M} < \infty
	\]
