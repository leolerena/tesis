...
\begin{deff}
	Sea $G$ un \fg y $A$ un conjunto de generadores. Definimos el \blue{grafo de Cayley} $Cay(G,A)$ como el grafo que tiene como vértices los elementos del grupo $G$ vistos como palabras en $A$ y las aristas etiquetadas con elementos de $A$. 
\end{deff}
...
\begin{deff}
El \blue{bagsize} de una descomposición en un árbol de un grafo $X$ es el siguiente valor:
	\begin{equation*}
...
\red{No sé si hace falta pero agregar lo del cíclico como ejemplo para diferenciar el caso que la descomposición es conexa o no. Esto depende de si lo necesito usar después en la demostración de cuasisometría.}
...
\begin{ej}\label{desc-grafo-cayley}%[Descomposición válida para todo grupo finitamente generado].
	Consideremos el grafo de $\Gamma = Cay(G,A)$ para cierto conjunto de generadores $\Sigma$ finito.
...
Sean $g,h \in \beta C$ para cierta $C$ componente conexa de $V_n$, veamos de acotar $d(g,h)$. 
