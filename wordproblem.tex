...
\begin{deff}
	...
	Supongamos que $1 \notin \Sigma$.
	Definimos el \blue{grafo de Cayley} $\Gamma = Cay(G,\Sigma)$ como el grafo que tiene como vértices $V(\Gamma) = G$ y $(g,h) \in E(\Gamma)$ sii $h=ga$ para ciertos $g,h \in G$ y $a \in \Sigma \cup \Sigma^-1$. 
\end{deff}
...
\begin{deff}
	...
	\begin{enumerate}
	\item Para todo par de puntos $x_1,x_2 \in X$ existen constantes $A,B \ge 0$ tales que
	\[
...
\red{En el caso que los espacios métricos son grafos podemos definir la cuasisometría sobre los vértices!! 
Esto facilita algunos argumentos molestos en los que la imágen de un vértice cae en el medio de una arista y cosas así.}
...
\medskip
\begin{ej}
	% El ejemplo clásico de la grilla y el plano.
\end{ej}
...
\begin{enumerate}
	...
	\red{Comentarios:
	La demo venía bien pero estoy asumiendo que $diam(X_t) \le k$ uniformemente sobre los bolsones. 
	Esto en principio no es cierto.
	Otra cosa es que si hago funcionar esto debería modificar las constantes pero creo que eso se puede tomar el máximo de las que usé y ya está.
	Creo que si pruebo lo que quería probar para otra cosa y que sé que es cierto:
	\textsc{la descomposición en un árbol se puede tomar de manera que los bolsones sean conexos} entonces en este caso notamos que $diam(X_t) \le k$ donde $k$ es horriblemente más grande pero finito.
	}
\end{enumerate}
