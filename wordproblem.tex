...
\begin{prop}\label{prop_vls}
	...
	\begin{enumerate}
		...
		\item Si $H$ es un subgrupo de $G$ entonces también resulta ser \vl.
	\end{enumerate}
\end{prop}

\begin{proof}
	\begin{enumerate}
		\item En particular si $G$ es virtualmente libre y $F$ es un subgrupo libre tenemos que la cantidad de conjugados de $F$ es finita por el lema \ref{lema_normalizador_conjugados}.
		...
		\item Si $F$ es un libre de índice finito en $G$ podemos tomar $H \cap F$ que es libre por ser subgrupo de un libre de acuerdo al resultado \ref{coro_niels_sch}.
...
\begin{proof}
	Sea $G$ grupo \vl \ presentado por $G  =\langle \Sigma  \mid  R \rangle$.
	Veamos de construir un automáta de pila de manera que acepta al lenguaje $\WP$.
