...
Veamos que todo grupo \vl \ se puede tomar de manera que el subgrupo sea normal.
\begin{deff}
...
En particular si $G$ es virtualmente libre y $F$ es un subgrupo libre tenemos que la cantidad de conjugados de $F$ es finita por lo tanto podemos considerar el siguiente subgrupo normal
\[
...
Primero probemos que tiene índice finito, para eso consideremos el siguiente resultado.
\begin{lema}
...
\begin{proof}
	...
	$[G:K\cap H] = [G:K][K: K \cap H]$ y como ambos índices de la derecha son finitos por lo visto obtenemos que $K \cap H$ es un subgrupo de índice finito también tal como queríamos ver.
\end{proof}

Con este resultado y haciendo inducción obtenemos que $F'$ tiene índice finito.
Nos quedaría ver que es libre.
Para eso usamos el resultado \ref{coro_niels_sch} que nos dice que todo subgrupo de un grupo libre es libre, en particular al ser $F'$ subgrupo de $F$ que es libre obtenemos que $F'$ es libre tal como queríamos ver.

Con esto probamos que podemos tomar al subgrupo de un \vl \ para que sea normal.
...
\begin{enumerate}
	...
	Sea $F$ un subgrupo que es isomorfo a $\ZZ$ y que tiene índice finito en $\ZZ \times \ZZ$.
	Sea $(n,m) \in \ZZ \times \ZZ$ el generador de $F$ de manera que $F = \langle $
...
\end{teo}
\todo[inline]{Escribir mejor lo que está dicho como sist de reescritura, quizá con Todd Coxeter puede andar.}
\begin{proof}
