...
\begin{deff}
Un grupo $G$ finitamente generado es \blue{virtualmente libre} si existe un subgrupo $F$ libre tal que su índice en $G$ es finito.
\end{deff}
...
\begin{prop}\label{prop_vls}
	...
	\begin{enumerate}
		\item Si $F$ es un subgrupo libre de índice finito entonces podemos tomarnos otro subgrupo $F'$ de manera que sea normal y libre.
...
\end{deff}
Como corolario de la proposición \ref{prop-cono-wp} y de saber que los lenguajes \ic \ forman un cono \ref{ic-cono} la definición anterior no depende del conjunto de generadores.

Veamos algunos ejemplos de grupos de esta familia.
\begin{ej}
...
\begin{teo}[Muller-Schupp 1983]\cite{}
...
\begin{proof}
	...
	Veamos de construir un autómata de pila de manera que acepta al lenguaje $\text{WP}(G,W)$.
	...
	\[
	{\cal M }= (T, Y , Z, \delta, 1, \{1\}, K)
	\]
	donde $T$ es el conjunto de los transversales que tomamos para la presentación de $G$ y van a ser los estados del autómata $M$.
	El alfabeto $Y$ es el del grupo libre $F$.
	Nuestro estado inicial que también es final corresponde a $1$ que es uno de los elementos del transversal.
	La constante la tomamos como $K = \max |u_{ij},\ z_{ijk}|$ donde $1 \le i,j,k  \le n$. 
	...
	\end{align*}
	donde el producto en el grupo libre $\cdot$ para ir reduciendo la palabra de la pila.
	Sabemos que hacemos tantas reducciones como $|u_{ij}|$ o bien $| z_{ijk}|$ y ambas valores están acotados por la constante $K$.
	...
	\[
		{\cal L }(M) = \{  w \in W^* \mid (w,1,\lambda)   \vdash^*  (\lambda, 1, 1)  \}
	\]
	...
	Esto es que cuando termina de consumir la cadena de entrada llegamos a la configuración $(\lambda, t_i, v)$ es decir que $w = vt_i$.
...
\begin{ej} Consideremos los siguientes ejemplos que nos irán guiando para entender todas las equivalencias.	
	\begin{enumerate}[E1.]
		\item Dado un grupo libre $F_\Sigma$ veamos cómo construir un automáta $\cal M$ tal que acepta su problema de la palabra. 
		...
		\item Para $C_2 \ast C_3$ es un autómata de 12 estados.
	\end{enumerate}
...
\begin{deff}
	...
	\begin{itemize}
	\item[\textbf{Q1.}] Para todo par de puntos $x_1,x_2 \in X$ existe constante $A > 0$ tal que
	\[
