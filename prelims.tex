...
\begin{obs}
	Bajo esta definición de un \APND notemos que no hay transición posible si en el tope de la pila está el símbolo inicial de la pila. \todo{mover esta obs más abajo}
\end{obs}


\begin{obs}
	En general en vez de dar una función de transición $\delta$ daremos una función parcial.
	Esto es que si $\delta(q,a,z)$ no está definido entonces implicitamente quiere decir que existe un estado $q_{t}$ denominado \emph{estado trampa} tal que $\delta(q,a,z) = (q_{t}, z)$.
	A su vez este estado cumple que...
	\todo{Escribir bien esta observación.}
