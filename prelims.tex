...
\todo[inline]{revisar a partir de acá.}
\begin{deff}
	Sea $G$ grupo finitamente generado por $A$ tal que $1 \notin A$.
	Sea $\text{Cay}(G,A)$ entonces definimos la siguiente función 
	$l: \Pi(\text{Cay}(G,A)) \to (A \cup A^{-1})^*$
	de la siguiente manera:
	si $c = (g_{0}, \dots), g_{n}) \in \Pi(\text{Cay}(G,A))$ entonces para cada $0 \le i < n$ consideramos $a_{i} \in A \cup A^{-1}$ tal que $g_{i} = g_{i-1}a_{i}$ y así podemos finalmente definir $l(c) = a_{0}\dots a_{n-1}$.
\end{deff}
\begin{lema}\label{obs_grafo_Cayley_palabras}
	Sea $G$ grupo finitamente generado por $A$ tal que $1 \notin A$.
	Sea $\text{Cay}(G,A)$ el grafo de Cayley para estos generadores y $l: \Pi(\text{Cay}(G,A)) \to (A \cup A^{-1})^*$ y sea $\pi:(A \cup A^{-1})^* \to G$ la proyección de monoides entonces vale lo siguiente:
	\begin{enumerate}
		\item Si $c$ es un camino cerrado luego $\pi(l(c)) = 1$.
...
\begin{proof}
	...
	Para eso si $c$ es un ciclo en $\Gamma$ entonces por la observación \ref{obs_grafo_Cayley_palabras} obtenemos una palabra $c$ en $(A \cup A^{-1})^*$ tal que es {reducida} y tal que $c \underset{G}{=} 1$.
