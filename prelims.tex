...
\begin{proof}
		...
		En el caso que $w \in L(\cal M)$ notemos que necesariamente para ser aceptada al consumirla tiene que llegar al estado $q_{2}$.
...
%	Existe $L$ lenguaje tal que existe $\cal M$ \APD de manera que $L({\cal M}) = L$ pero no existe ningún ${\cal M}'$ tal que $L = N({\cal M}')$.
%\end{prop}
%\begin{proof}
%	Consideremos el lenguaje $L = \{ a^m b^n  : m \ge n \ge 1 \}$.
%	
...
\begin{proof}
	...
	\[
		(g_{0}a_{0})(a_{2} \dots a_{n-1}) = g_{1}(a_{2} \dots a_{n-1}) = \dots = g_{0}.
	\] 
	y esto dice que $g_{0}(a_{0} \dots a_{n-1}) = g_{0}$ pero por lo visto en la proposición \ref{lema_grp_acc_libre_cayley} el grupo $G$ actúa libremente sobre su grafo de Cayley, por lo tanto el estabilizador de todo vértice tiene que ser trivial.
	Esto nos dice que $l(c) = 1$ tal como queríamos ver.
	
	Probemos \textbf{2}.
	Si $c = (g_{0}, \dots, g_{n})$ es un ciclo tal que $l(c) = (a_{0}, \dots, a_{n-1})$ luego va a ser reducida porque caso contrario existiría $i$ tal que $a_{i}^{-1} = a_{i+1}$ lo que implica que $g_{i}a_{i} = g_{i+1}$ y que $g_{i+1}a_{i}^{-1} = g_{i+2}$ y de esta manera obtenemos que $g_{i+2}a_{i} = g_{i+1} = g_{i}a_{i}$ y así concluimos que $g_{i+2} = g_{i}$ por lo que el camino $c$ no es un ciclo.
