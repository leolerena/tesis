...
Finalmente en \ref{} introducimos la notación que usaremos para los grafos no dirigidos, introduciremos los grafos de Cayley y probaremos un resultado \ref{} sobre los grafos de Cayley de los grupos libres que nos servirá más en adelante para guiarnos en la caracterización de los grafos de Cayley de los grupos virtualmente libres.
...
\begin{deff}
	...
	\begin{itemize}
		...
		\item $\delta$ es la \emph{función de transición} \todo[]{estados trampa o bien función parcial.} donde $\delta: Q  \times \Sigma \cup \{ \lambda \} \times \Gamma \cup \{ \lambda \} \to {\cal P}( Q  \times \Gamma)$;
		\item $q_0 \in Q$ es el \emph{estado inicial};
		\item $F \subseteq Q$ es el conjunto de \emph{estados finales};
		\item $Z_{0}$ es el \emph{símbolo inicial} de la pila y lo tomamos para que $Z_{0} \notin \Gamma$.
	\end{itemize}
\end{deff}
...
Consideremos la relación generada por la clausura transitiva y reflexiva de esta operación.
Así como en el caso anterior de los autómatas finitos la denotaremos $\vdash^*_{\cal M}$.
...
\begin{ej}
	...
	\[
		...
		\begin{cases}
			...
			(q_0,\lambda) 	&  \text{si} \ i=1, Y = a, Z = a 	\\
			(q_0,\lambda) 	&  \text{si} \ i=1, Y = b, Z = b \\
			(q_1,Z)			& \text{si} \ i=0, Y = \lambda \\
			(q_1, Z)		& \text{si} \ i=0 \\
		\end{cases}
	\]
	\todo[]{Justificar formalmente con doble inclusión. }
...
Introduciremos la notación $F_{k}$ para referirnos a un grupo libre generado por algún conjunto $S$ tal que $|A| = k$.
...
\begin{lema}
	Sea $G$ un grupo y sean $K,H$ subgrupos de índice finito entonces $K \cap H$ es un subgrupo de índice finito.
\end{lema}
\begin{proof}
...
A partir del resultado anterior con una inducción directa podemos probar el siguiente resultado.
		\todo[]{Juntar este lema con el anterior.}
...
\end{proof}
\todo[inline]{Pasar todo al cap de grupos ic.}
\begin{deff}
...
\todo[]{Hablar de caminos en grafos.}
...
\end{deff}
\begin{obs}
	Todo grafo $\Gamma$ de grado acotado uniformemente es localmente finito.
\end{obs}
...
\begin{deff}
	...
	Definimos el \emph{grafo de Cayley} $\Gamma = \text{Cay}(G,A)$ como el grafo que tiene como vértices $V(\Gamma) = G$ y aristas $\{g,h\} \in E(\Gamma)$ si y solo sí $h=ga$ para ciertos $g,h \in G$ y $a \in A \cup A^{-1}$. 
	
	Para una arista $e = \{g,ga\}$ llamaremos la \emph{etiqueta} de $e$ al generador $a \in A \cup A^{-1}$.
\end{deff}
...
\begin{obs}
	Sea $G$ grupo finitamente generado por $A$. 
	Sea $\Gamma = Cay(G,A)$ luego $\Gamma$ tiene grado acotado uniformemente por $|A|$.
\end{obs}
...
\todo[inline]{Agregar lo básico de grafos de Cayley y probar grafo de Cayley de libre es árbol}
