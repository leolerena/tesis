...
Las \emph{configuraciones finales} van a ser las configuraciones que tienen la pinta $(q,\epsilon,\gamma)$ donde $q \in F$.
Una manera de \emph{consumir} una palabra $w \in \Sigma^*$ es una sucesión finita de configuraciones $(q_{0},w,Z_{0}) \vdash \dots \vdash (p,u,\theta)$ donde $(p,u,\theta)$ es una configuración irreducible.
...
Dicho de otra manera el lenguaje aceptado por un autómata de pila no determinístico son las palabras que pueden ser consumidas de manera que terminan en un una configuración final.

\begin{lema}
	Sea ${\cal M} = (Q, \Sigma, \Gamma, \delta, q_0, F, Z_0)$ un \APND y 
	sean $w_1, w_2 \in \Sigma^*$, $\gamma \in \Gamma^*$ entonces
	\[
		(q_{0},w_{1}, Z_{0}) \vdash^* (q, \epsilon, \gamma) \iff (q_{0},w_{1}w_{2}, Z_{0}) \vdash^* (q,w_{2}, \gamma).
	\]
\end{lema}
\begin{proof}
	\todo[inline]{HACER.}
...
\begin{ej}\label{ej_ic_palindromos}
	...
	Al autómata de pila lo representaremos de la siguiente manera:
	\begin{center}
		\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=3.5cm,
			...
			\path (q_1) edge [bend right] node {} (q_2);
		\end{tikzpicture}
	\end{center}
